...
use crate::target::{ArmTarget, Target, X86MemoryLevel};
...
impl<'a> CpuCodeGenerator<'a, ArmTarget> {
    fn emit_kernel<W: Write, Aux: Clone + Debug>(
        &mut self,
        imp: &'a ImplNode<ArmTarget, Aux>,
        top_arg_tensors: &'a [Rc<Tensor<ArmTarget>>],
        out: &mut W,
    ) -> fmt::Result {
        ...
        let tensors_as_trait_obj_ptrs = top_arg_tensors
            .iter()
            .map(|tensor| tensor.as_ref() as &dyn View<Tgt = ArmTarget>)
            .collect::<Vec<_>>();
    ...
    fn param_args_to_c_indices<F>(&self, arguments: &[Param<ArmTarget>], f: F) -> Vec<String>
    where
