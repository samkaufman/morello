...
def test_convolution_image_tile_boundary_size(
    ...
    any_level = "GL"
    outer = tensor.Tensor(
        specs.TensorSpec(outer_shp, dtypes.Uint8, True, True, any_level, layouts.row_major(len(outer_shp))),
        name=None,
    )
    tile = tensor.ConvolutionImageTile(
        tensor.OperandIdx(0),
        specs.TensorSpec(tile_shp, dtypes.Uint8, True, True, any_level, layouts.row_major(len(tile_shp))),
