...
@hypothesis.given(_arb_test_bcarray_fills_result_array_matching_ndarray_args())
def test_bcarray_to_dense_returns_correct_shape(example):
    (_, _, _, bs_arr, _) = example
    assert bs_arr.to_dense().shape == bs_arr.shape


@hypothesis.given(_arb_test_bcarray_fills_result_array_matching_ndarray_args())
def test_bcarray_iter_values_returns_set_present_in_dense(example):
    (_, _, _, bs_arr, _) = example
    assert set(bs_arr.iter_values()) == set(bs_arr.to_dense().flatten().tolist())
