from morello import tensor, ops, specs, replace


def test_replace_origin_of_tiles():
    origin = tensor.Tensor(
        specs.TensorSpec((10, 10), level=1), name="origin", origin=None
    )
    l_and_r = origin.simple_tile((4, 4))
    output = origin.simple_tile((4, 4))
    impl = ops.Matmul(l_and_r, l_and_r, output, serial_only=False)

    new_origin = tensor.Tensor(
        specs.TensorSpec((20, 20), level=1), name="new_origin", origin=None
    ).simple_tile((10, 10))
...
def test_replace_is_no_op_with_no_real_changes():
    ...
    impl = ops.Matmul(lhs, rhs, output, serial_only=False)
