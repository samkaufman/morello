...
///
/// For register-counting memories (e.g., VRF, RF), values are stored exactly as integers.
/// For non-register-counting memories (e.g., L1, GL), values are rounded down to the
...
impl MemoryLimits {
    ...
    ///
    /// For Pipeline variants, computes `snap(limits - before - after)` per memory.
    ...
    /// Snap all non-register memories down to the nearest power of two.
    ///
    ...
    pub fn discretize<Tgt: Target>(&mut self) {
        match self {
            ...
            MemoryLimits::Pipeline {
                ...
                for (i, memory) in Tgt::memories().into_iter().enumerate() {
                    if !memory.counts_registers() {
    ...
    fn transition_pipeline<Tgt: Target>(
        &self,
        intermediate_consumption: &[[u64; MEMORY_COUNT]],
    ) -> Option<Vec<MemoryLimits>> {
...
impl<Tgt: Target> BiMap for MemoryLimitsBimap<Tgt> {
    ...
    fn apply(&self, t: &Self::Domain) -> Self::Codomain {
        let memories_count = Tgt::memories().len();
        let mut result = vec![0u32; 1 + 3 * memories_count];
            ...
            MemoryLimits::Pipeline {
                ...
                for (i, memory) in Tgt::memories().into_iter().enumerate() {
                    if memory.counts_registers() {
                        ...
                        result[1 + memories_count + i] = before[i].try_into().unwrap();
                        result[1 + 2 * memories_count + i] = after[i].try_into().unwrap();
                    } else {
                        result[1 + i] = BimapInt::from(limits.get_binary_scaled(i));
                        result[1 + memories_count + i] = bit_length(before[i]);
                        result[1 + 2 * memories_count + i] = bit_length(after[i]);
                    }
                }
            }
        }
    ...
    fn apply_inverse(&self, i: &Self::Codomain) -> Self::Domain {
        let memories = Tgt::memories();
        if i.len() != 1 + 3 * memories.len() {
            panic!("Invalid MemoryLimits BiMap vector length");
        }

        match i[0] {
            0 => {
                let limits_data = &i[1..1 + memories.len()];
                let mut values = ZERO_LIMITS;
                let mut use_raw_encoding = [false; MEMORY_COUNT];
            ...
            1 => {
                let limits_data = &i[1..1 + memories.len()];
                let before_data = &i[1 + memories.len()..1 + 2 * memories.len()];
                let after_data = &i[1 + 2 * memories.len()..1 + 3 * memories.len()];
...
pub fn arb_memorylimits_ext<Tgt: Target>(
    ...
    component_ranges.prop_flat_map(move |v| {
        ...
        prop_oneof![
            ...
            3 => {
                // Generate before/after such that before and after sum to at most
                // limits on each memory.
...
mod tests {
    ...
    fn test_transition_standard_to_pipeline() {
        ...
        assert_eq!(
            standard_unscaled_memories(&children[0]),
            [56, 28, 1024, 2048]
        );
        assert_eq!(
            standard_unscaled_memories(&children[1]),
            [40, 20, 1024, 2048]
        );
        assert_eq!(
            standard_unscaled_memories(&children[2]),
            [48, 24, 1024, 2048]
        );
    }
    ...
    proptest! {
        ...
        /// to the same per-memory capacity, `snap(initial - outer_buf)`.
        #[test]
        fn test_nested_pipeline_outer_into_standard_composition(
            ...
            let outer_expected = array::from_fn(|i| {
                ...
                if Avx2Target::memories()[i].counts_registers() {
                    remaining
                } else {
            ...
            for outer_child in &outer_children {
                prop_assert_eq!(standard_unscaled_memories(outer_child), outer_expected);
            }
        }
        ...
        fn test_nested_pipeline_inner_into_standard_composition(
            ...
            let avx2_memories = Avx2Target::memories();
            let inner_expected = array::from_fn(|i| {
                ...
                if avx2_memories[i].counts_registers() {
                    after_inner
                } else {
            ...
            for outer_child in &outer_children {
                ...
                for inner_child in &inner_children {
                    prop_assert_eq!(standard_unscaled_memories(inner_child), inner_expected);
                }
            }
        }
        ...
        fn test_nested_pipeline_limits_preserved_through_nesting(
            ...
            for outer_child in &outer_children {
                ...
                for memory in 0..MEMORY_COUNT {
                    prop_assert_eq!(
                        outer_limits.get_unscaled(memory),
                        initial.get_unscaled(memory),
                        "outer limits changed at memory {}",
                        memory
                    );
                }
                ...
                for inner_child in &inner_children {
                    ...
                    for memory in 0..MEMORY_COUNT {
                        prop_assert_eq!(
                            inner_limits.get_unscaled(memory),
                            initial.get_unscaled(memory),
                            "inner limits changed at memory {}",
                            memory,
                        );
                    }
                }
        ...
        fn test_nested_pipeline_before_after_nondecreasing(
            ...
            for outer_child in &outer_children {
                ...
                for inner_child in &inner_children {
                    ...
                    for memory in 0..MEMORY_COUNT {
                        prop_assert!(inner_before[memory] >= outer_before[memory]);
                        prop_assert!(inner_after[memory] >= outer_after[memory]);
                    }
                }
            }
        }
    ...
    fn standard_unscaled_memories(limits: &MemoryLimits) -> [u64; MEMORY_COUNT] {
