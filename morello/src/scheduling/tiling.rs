            ...
            LogicalSpec::Compose { .. } => {
                if MULTI_DIM_TILING || !spec.0.serial_only() {
                    let tileout = TileOut::MultiLoop {
                        output_shape: smallvec![nz!(1u32); output_shape.len()],
                        parallel: false,
                    };
...
fn spec_canonicalize_to_apply_err(canon_error: crate::spec::CanonicalizeError) -> ApplyError {
    match canon_error {
        crate::spec::CanonicalizeError::TensorSpecAuxCanonicalizeError(_) => {
            ApplyError::NotApplicable(NotApplicableReason::TileShapeInvalid)
        }
        crate::spec::CanonicalizeError::SideEffectingComponent => {
...
/// Map the rectangles defined by `multidim_cutoffs` to the [Layout] for each argument
...
fn compute_layout_contigs_for_cutoff_rects<'a>(
    ...
    multidim_cutoffs
        ...
        .map(move |dims| {
            ...
            for (parent_shape, layout, dim_associations) in tensor_descriptions {
                ...
                let representative_tile_shape = dim_associations
                    ...
                    .collect::<Vec<_>>();
                match layout.update_for_tiling(parent_shape, &representative_tile_shape) {
                    Ok(new_layout) => value.push(new_layout),
                    Err(_) => break,
                };
            }
...
mod tests {
    ...
    fn test_compute_layout_contig_rects_1() {
        ...
        let tensor_descriptions = vec![(&spec_shape[..], &rm, &da[..])];
        let fixed_spec_dimensions = &[];

        let mut rmc1 = row_major(2);
        rmc1.set_contig(1);

        // TODO: Construct an R-Tree rather than testing for the specific output rectangles.
        let expected = HashSet::from([
            ((vec![1, 1]).clone(), (vec![1, 1]).clone(), vec![rm.clone()]),
            (vec![2, 1], vec![5, 1], vec![rmc1.clone()]),
            (vec![1, 2], vec![1, 5], vec![rm.clone()]),
            (vec![2, 6], vec![5, 6], vec![rm.clone()]),
            (vec![6, 1], vec![6, 1], vec![rmc1.clone()]),
            (vec![6, 2], vec![6, 5], vec![rmc1.clone()]),
            (vec![2, 2], vec![5, 5], vec![rmc1.clone()]),
            (vec![1, 6], vec![1, 6], vec![rm.clone()]),
            ...
            for action in actions {
                match action.apply(&spec) {
                    Ok(applied_impl) => {
                        ...
                        applied_impl.visit_leaves(&mut |leaf| {
                        ...
                        for (idx, layout) in located.unwrap().iter().enumerate() {
                            ...
                            prop_assert_eq!(layout.contig(), expected_contig);
                        }
                    },
                    Err(err) => {
