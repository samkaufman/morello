...
mod tests {
    ...
    fn test_plan_alloc_preserves_pipeline_structure() {
        ...
        const BEFORE: [u64; MEMORY_COUNT] = [0, 0, 512, 512];
        const AFTER: [u64; MEMORY_COUNT] = [0, 0, 256, 256];
        ...
        // limits should be unchanged at levels 0, 1, 3; decreased at L1 (level 2)
        ...
        assert_eq!(result_limits.get_unscaled(3), limits.get_unscaled(3));

        // before and after should be unchanged.
        assert_eq!(*result_before, BEFORE);
