...
mod tests {
    ...
    proptest! {
        ...
        #[test]
        fn test_move_actions_never_returns_within_memory_copy(
    ...
    /// Returns the per-memory effective available memory for `memory_limits`,
    /// or `None` if `memory_limits` is invalid (e.g. pipeline limits where `before
    /// + after > limits` at any memory).
    ///
    /// For `Standard`, this is the raw limit at each memory. For `Pipeline`,
    /// this computes `limits - before - after` per memory and returns `None`
    /// if any subtraction underflows.
    ///
    /// Unlike [`MemoryLimits::into_standard`], this is lossless for non-register
    /// memories (no power-of-two snapping) and uses checked subtraction rather
    /// than saturating underflow to zero.
    fn memory_limits_available(memory_limits: &MemoryLimits) -> Option<[u64; MEMORY_COUNT]> {
        match memory_limits {
            ...
            MemoryLimits::Pipeline {
                ...
                let mut available = [0; MEMORY_COUNT];
