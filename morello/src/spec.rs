...
impl PrimitiveBasics {
    pub fn replace_io(&mut self, new_operands: &[(&[DimSize], Dtype)]) {
        ...
        for (operand, associations) in new_operands.iter().zip(all_associations) {
            for (association, size) in associations.iter().zip(operand.0) {
                if let DimAssociation::SpecDimSum(lhs_dim, rhs_dim, shift) = association {
                    let lhs = i64::from(self.spec_shape[*lhs_dim].get());
                    let rhs = i64::from(self.spec_shape[*rhs_dim].get());
                    let result = u32::try_from(lhs + rhs + i64::from(*shift)).unwrap();
                    let expected = DimSize::new(result).unwrap();
    ...
    pub fn parameter_shape(&self, idx: usize) -> Shape {
        self.spec_dim_associations(idx.try_into().unwrap())
            ...
            .map(|a| match a {
                ...
                DimAssociation::SpecDimSum(lhs_dim, rhs_dim, shift) => {
                    let lhs = i64::from(self.spec_shape[lhs_dim].get());
                    let rhs = i64::from(self.spec_shape[rhs_dim].get());
                    let result = u32::try_from(lhs + rhs + i64::from(shift)).unwrap();
                    DimSize::new(result).unwrap()
                }
    ...
    fn spec_dim_associations(&self, parameter_idx: u8) -> Vec<DimAssociation> {
        match self.typ {
            ...
            PrimitiveSpecType::Conv { accum: _ } => {
                let [b, f, c, h_add, w_add, fh, fw] = [0, 1, 2, 3, 4, 5, 6];
                match parameter_idx {
                    0 => {
                        vec![
                            DimAssociation::SpecDim(b),
                            DimAssociation::SpecDim(c),
                            DimAssociation::SpecDimSum(h_add, fh, -1),
                            DimAssociation::SpecDimSum(w_add, fw, -1),
                        ]
                    }
                    1 => sdims![f, c, fh, fw],
                    2 => sdims![b, f, h_add, w_add],
