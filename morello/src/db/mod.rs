...
impl FilesDatabase {
    ...
    /// Looks up optima for canonical [Spec]s that may span multiple pages.
    ///
    /// Each query carries a caller-provided identifier. Returned results keep
    /// that identifier so callers can route hits and misses without building a
    /// side table keyed by query point.
    pub(crate) fn get_many_pages_canon_indexed<Tgt>(
        &self,
        queries: Vec<(usize, &Spec<Tgt>)>,
    ) -> Vec<(usize, GetPreference<ActionCostVec, Vec<ActionNum>>)>
    where
        Tgt: Target,
        Tgt::Memory: CanonicalBimap,
        <Tgt::Memory as CanonicalBimap>::Bimap: BiMap<Codomain = u8>,
    {
        let bimap = self.spec_bimap();
        let mut by_table = HashMap::<TableKey, Vec<(usize, &Spec<Tgt>, Vec<BimapSInt>)>>::new();
        for (query_idx, query) in queries {
            debug_assert!(query.is_canonical());
            let (table_key, global_pt) = BiMap::apply(&bimap, query);
            by_table.entry(table_key).or_default().push((
                query_idx,
                query,
                global_pt.into_iter().map(BimapSInt::from).collect(),
            ));
        }

        let mut results = Vec::new();
        for (table_key, queries) in by_table {
            results.extend(self.get_many_pages_canon_indexed_for_table(table_key, queries));
        }
        results
    }

    fn get_many_pages_canon_indexed_for_table<Tgt>(
        &self,
        table_key: TableKey,
        queries: Vec<(usize, &Spec<Tgt>, Vec<BimapSInt>)>,
    ) -> Vec<(usize, GetPreference<ActionCostVec, Vec<ActionNum>>)>
    where
        Tgt: Target,
    {
        let Some((_, _, first_point)) = queries.first() else {
            return Vec::new();
        };
        let rank = first_point.len();
        debug_assert!(queries.iter().all(|(_, _, point)| point.len() == rank));

        self.get_many_pages_canon_indexed_for_table_rects(table_key, queries, rank)
    }

    fn get_many_pages_canon_indexed_for_table_rects<Tgt>(
        &self,
        table_key: TableKey,
        queries: Vec<(usize, &Spec<Tgt>, Vec<BimapSInt>)>,
        rank: usize,
    ...
    where
        Tgt: Target,
    {
        let mut specs_by_query_idx = HashMap::with_capacity(queries.len());
        let mut points_by_query_idx = HashMap::with_capacity(queries.len());
        let mut results = queries
            .iter()
            .map(|(query_idx, spec, point)| {
                specs_by_query_idx.insert(*query_idx, *spec);
                points_by_query_idx.insert(*query_idx, point.clone());
                (*query_idx, GetPreference::Miss(None))
            })
            .collect::<HashMap<_, _>>();

        let mut rects_by_page =
            HashMap::<Vec<BimapInt>, (Vec<BimapSInt>, Vec<BimapSInt>, SmallVec<[usize; 1]>)>::new();
        for (query_idx, _, point) in &queries {
            let page_point = blockify_point(point);
            let entry = rects_by_page
                .entry(page_point)
                .or_insert_with(|| (point.clone(), point.clone(), SmallVec::<[usize; 1]>::new()));
            for (bottom, &coord) in entry.0.iter_mut().zip(point) {
                *bottom = (*bottom).min(coord);
            }
            for (top, &coord) in entry.1.iter_mut().zip(point) {
                *top = (*top).max(coord);
            }
            entry.2.push(*query_idx);
        }

        let query_tree = RTreeDyn::bulk_load(rank, rects_by_page.into_values().collect());
        ...
        for (query_bottom, query_top, _) in query_tree.iter() {
            ...
            multi_range_product(&page_bottom, &page_top, |page_point: &[BimapInt]| {
                ...
                let page_key = self
                    .prehasher
                    .prehash((table_key.clone(), page_point.to_vec()));
                ...
                query_tree
                    ...
                    .for_each(
                        |(
                            (query_bottom, query_top, query_indices),
                            (memo_bottom, memo_top, memo_value),
                        )| {
                            ...
                            for &query_idx in query_indices {
                                let query_point = points_by_query_idx
                                    .get(&query_idx)
                                    .expect("query tree returned an unknown query index");
                                if !point_in_rect(query_point, &overlap_bottom_pt, &overlap_top_pt)
                                {
                                    continue;
                                }
                                let query = specs_by_query_idx
                                    .get(&query_idx)
                                    .expect("query tree returned an unknown query index");
                                results.insert(
                                    query_idx,
                                    GetPreference::Hit(memo_value_to_action_cost_vec(
                                        query, memo_value,
                                    )),
                                );
                            }
        ...
        queries
            ...
            .map(|(query_idx, _, _)| {
                (
                    ...
                    results
                        .remove(&query_idx)
                        .expect("all queries should have an initialized result"),
                )
            })
            .collect()
    }
...
fn point_in_rect(point: &[BimapSInt], bottom: &[BimapSInt], top: &[BimapSInt]) -> bool {
    debug_assert_eq!(point.len(), bottom.len());
    debug_assert_eq!(point.len(), top.len());
    point
        .iter()
        .zip(bottom.iter().zip(top))
        .all(|(&point, (&bottom, &top))| bottom <= point && point <= top)
}
