...
mod tests {
    ...
    /// but with one memory limit increased.
    ///
    ...
    fn arb_lower_and_higher_canonical_specs<Tgt: Target>(
        ...
        (
            ...
            .prop_filter_map(
                "chosen memory must support a valid memory adjustment",
                move |(spec, idx)| {
    ...
    /// Yields [MemoryLimits] which have the 'base' memory at `idx` raised.
    ///
    ...
    fn arb_raised_available<Tgt: Target>(
        ...
        let raised_limit = if Tgt::memories()[idx].counts_registers() {
        ...
        let use_raw_encoding = Tgt::memories().map(|m| m.counts_registers());
    ...
    fn arb_lower_pipeline_field<Tgt: Target>(
        ...
        let lowered_component = if Tgt::memories()[idx].counts_registers() {
            (0..current).boxed()
        } else {
