    ...
    <Tgt::Memory as CanonicalBimap>::Bimap: BiMap<Codomain = u8>,
{
    ...
    fn solve_canonical(&mut self, spec: &Spec<Tgt>) -> ActionCostVec {
        ...
        let memoizable = self.db.can_memoize_efficiently(spec);
        if memoizable {
...
mod tests {
    ...
    fn test_spatial_search_database_matches_top_down_for_small_move() {
        ...
        let reference_db = FilesDatabase::new::<Avx2Target>(None, TileScale::Linear, 1, 128, 1);
        let spatial_db = FilesDatabase::new::<Avx2Target>(None, TileScale::Linear, 1, 128, 1);

        let reference = search::top_down_many(&reference_db, slice::from_ref(&spec));
